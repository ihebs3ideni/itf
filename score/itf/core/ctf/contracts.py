...
class Provider:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "is_generator", inspect.isgeneratorfunction(self.factory))


def build_provider(fn: Callable[..., Any]) -> Provider:
    """Construct a Provider from a @provides-decorated factory."""
    contract = getattr(fn, _PROVIDES_ATTR, None)
    if contract is None:
        raise ValueError(f"{getattr(fn, '__name__', fn)!r} is not decorated with @provides(...)")
