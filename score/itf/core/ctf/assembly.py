...
from score.itf.core.ctf.lifecycle import instantiate_provider
...
def analyze(registry: Registry, resolver: GraphResolver, mode: RunMode) -> CompositionPlan:
    """Classify a registry into spine / available / unavailable under ``mode``.

    The spine is the union of ``@requires`` closures rooted at all declared
    anchors (contracts matching ``ctf/target`` or ``ctf/target/*``).  It must
    always resolve. In effective strict mode -- ``STRICT``, or ``LOOSE`` with no
    declared anchor -- any unmet provider also raises. In ``LOOSE`` with at
    least one anchor, unmet *additive* providers are recorded, not raised.
    """
    # Find all anchors in the registry
    anchors = frozenset(c for c in registry.contracts() if is_anchor(c))
    has_anchor = bool(anchors)

    # Spine = union of requires-closures of all anchors
    spine: set[str] = set()
    for anchor in anchors:
        spine.update(resolver.plan(anchor))
...
class Assembly:
    ...
    def __init__(self, registry: Registry, resolver: GraphResolver, plan: CompositionPlan) -> None:
    ...
    def get(self, contract: str) -> Any:
        ...
        """
        if not self._active:
            raise CompositionError(f"cannot resolve {contract!r}: the assembly is not active")
        if contract in self.plan.unavailable:
            raise CapabilityUnavailableError(contract, self.plan.unavailable[contract])
    ...
    def realize(self) -> tuple[TierReport, ...]:
        ...
        for tier in range(max_tier + 1):
            ...
            for contract in sorted(c for c in realize if self.plan.tier_of[c] == tier):
