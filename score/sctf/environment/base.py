...
This module defines the :class:`Environment` ABC and the :class:`ProcessHandle`
dataclass that together form the unified interface for all SCTF backends.
Tests written against this interface can transparently run on different
backends (Docker, Bubblewrap, bare-metal, etc.) by changing a single flag.
"""
...
class Environment(ABC):
    ...
    interface so that higher-level code can remain backend-agnostic.

    Lifecycle::

        env = DockerEnvironment(config)
        env.setup()
        handle = env.execute(path, args)
