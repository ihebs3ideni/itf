...
from ctf.contracts import Provider
...
def _make_finalizer(name: str, generator: GeneratorType) -> Callable[[], None]:
    def finalize() -> None:
        try:
            ...
            return
        raise CompositionError(
            f"generator provider {name!r} yielded more than once"
        )
...
def instantiate_provider(
    provider: Provider, args: Sequence[Any]
    ...
    if not isinstance(generator, GeneratorType):  # pragma: no cover - defensive
        raise CompositionError(
            f"provider {provider.name!r} was detected as a generator but did "
    ...
    try:
        ...
        raise CompositionError(
            f"generator provider {provider.name!r} did not yield a resource"
        ) from exc
