...
- **SCTF (Software Component Test Framework)**: Docker-based component testing with automatic OCI image packaging via `sctf_image()` and the `sctf_docker` plugin
...
SCTF uses the standard `py_itf_test` macro — there is no separate test macro. The key pieces are:

- **`sctf_docker`** — Static plugin struct that enables the Docker and SCTF pytest plugins
- **`sctf_image()`** — Bazel macro that builds an OCI image from your build artifacts
...
Build the image with `sctf_image()`, then pass `sctf_docker` as a static
plugin and wire the image via `args` / `data`:
...
Sharing one image across multiple tests is straightforward — just reference
the same tarball from each `py_itf_test`:
...
When `sctf_image()` is called, it automatically:
...
The `--docker-image-bootstrap` and `--docker-image` args are then wired
explicitly in each `py_itf_test` call.
